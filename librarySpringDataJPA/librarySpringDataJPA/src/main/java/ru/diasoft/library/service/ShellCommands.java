...
public class ShellCommands {
    ...
    @ShellMethod(value = "Find Author Book",  key = {"FAB", "Find Author Book"})
    public void FindBookByAuthor(String authorName) {
        Author author = authorService.getByName(authorName).get();
        if (authorService.getByName(authorName).isPresent()){
            if (bookService.getByAuthor(author).isEmpty())
                System.out.println("Книги не найдены!");

            else
                System.out.println(bookService.getByAuthor(author)
                        .stream().map(Book::getTitle).collect(Collectors.joining("; ")));
    ...
    @ShellMethod(value = "Find Genre Book",  key = {"FGB", "Find Genre Book"})
    public void FindBookByGenre(String genreName) {
        Genre genre = genreService.getByName(genreName).get();
        if (genreService.getByName(genreName).isPresent()){
            if (bookService.getByGenre(genre).isEmpty())
                System.out.println("Книги не найдены!");

            else
                System.out.println(bookService.getByGenre(genre)
                        .stream().map(Book::getTitle).collect(Collectors.joining("; ")));
