...
class BookServiceImplTest {
    ...
    void createTest() {
        when(authorRepositoryJpa.getByName(AUTHOR_NAME)).thenReturn(java.util.Optional.ofNullable(author));
        when(genreRepositoryJpa.getByName(GENRE_NAME)).thenReturn(java.util.Optional.ofNullable(genre));
        ...
        verify(authorRepositoryJpa, times(1)).getByName(AUTHOR_NAME);
        verify(genreRepositoryJpa, times(1)).getByName(GENRE_NAME);
    ...
    void getById() {
        when(bookRepositoryJpa.getById(BOOK_ID)).thenReturn(java.util.Optional.ofNullable(book));

        Book findBook = service.getById(BOOK_ID);

        assertEquals(findBook,book);

        verify(bookRepositoryJpa, times(1)).getById(BOOK_ID);
    }

    @Test
    @DisplayName("искать книгу по названию")
    void getByTitle() {
        when(bookRepositoryJpa.getByTitle(TITLE)).thenReturn(java.util.Optional.ofNullable(book));

        Optional<Book> findBook = service.getByTitle(TITLE);

        assertEquals(findBook.get(),book);

        verify(bookRepositoryJpa, times(1)).getByTitle(TITLE);
    ...
    void getAll() {
        when(bookRepositoryJpa.getAll()).thenReturn(books);

        List<Book> bookList = service.getAll();

        assertEquals(bookList,books);

        verify(bookRepositoryJpa, times(1)).getAll();
    }
